    ...
    fetch("https://disease.sh/v3/covid-19/countries")
        ...
        .then(rsp => {
            ...
            rsp.forEach(country => {
                ...
                const tests = (country.testsPerOneMillion/1000).toFixed(2);

                const el = document.createElement('div');
                active < 5 ? el.className = 'marker1': active < 20 ? el.className = "marker2" : active < 50 ? el.className = "marker3" :  active < 100 ? el.className = "marker4" : el.className = "marker5";

                 // make a marker for each feature and add to the map
                new mapboxgl.Marker(el).setLngLat([longitude, latitude]).setPopup(
                    new mapboxgl.Popup({ offset: 5, className: "popup2" }) // add popups
                      .setHTML(
                        `
                        <h5 >${title}</h5><p>Active: <b>${active} / 1k </b></p>
      ...
      function renderListings(features) {
        ...
        if (features.length) {
          for (const feature of features) {
            const label = feature.properties.province !== "null"
            ...
            itemLink.innerHTML =  feature.properties.province !== "null"
      ...
      function getUniqueFeatures(features, comparatorProperty) {
        ...
        const pulsingDot = {
          ...
          render: function () {
            ...
            context.fillStyle = `rgba(255, 165, 0, ${0.6 - t})`;
            context.fill();
